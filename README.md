...
| `/styles` | Per-chat стили: индивидуальный стиль для каждого чата (только для подключённых) |
...
Меню команд динамическое: `/connect` и `/disconnect` показываются в зависимости от статуса подключения. `/styles` видна только подключённым. `/start` не отображается в меню.
...
### Per-chat стили (`/styles`)

Команда `/styles` показывает последние 10 диалогов, кроме `Saved Messages`, с именем собеседника или названием чата и текущим стилем. Нажатие на кнопку циклически переключает стиль. Доступна только подключённым пользователям.
...
- **handlers/** — Обработчики команд (`bot_handlers.py` — `/start`, `on_text`; `settings_handler.py` — `/settings`; `styles_handler.py` — `/styles`) и событий Pyrogram (`pyrogram_handlers.py` — `/connect`, `on_pyrogram_message`, `on_pyrogram_draft`)
